...
Voice Recorder는 녹음 처리와 UI 표시 로직을 분리해 구성했습니다.  
녹음 상태, 오디오 데이터, Wave Bar 표시값, 미리듣기, 저장 흐름은 별도의 녹음 관리 계층에서 처리하고, Widget은 전달받은 상태값을 화면에 반영하는 역할에 집중하도록 구성했습니다.

AI 보조로 작성한 녹음 처리 초안을 프로젝트 구조에 맞게 연결하고, Android 모바일 UI 흐름에 맞춰 녹음 시작 / 정지 / 미리듣기 / 저장 단계를 적용했습니다.

### Implementation

| Step | Description |
|------|-------------|
| **1. Recording** | 마이크 입력을 받아 녹음 데이터를 누적 |
| **2. Wave Bar** | 입력 음량을 시각화 데이터로 변환해 녹음 중 Wave Bar에 반영 |
| **3. Time Control** | 최소 / 최대 녹음 시간 조건에 따라 정지 가능 여부와 자동 종료 처리 |
| **4. Preview** | 녹음 완료 후 사용자가 결과를 확인할 수 있도록 미리듣기 제공 |
| **5. Save** | Confirm 시 녹음 결과를 WAV 파일로 저장 |
| **6. UI Update** | 녹음 상태, 시간, 미리듣기 상태, Wave Bar 값을 Widget에 전달해 화면 갱신 |

### Highlights

- 녹음 처리 계층과 Widget UI 계층을 분리
- Android 음성 녹음 흐름을 모바일 UI에 맞게 연결
- 녹음 중 입력 음량을 Wave Bar 형태로 시각화
- 최소 / 최대 녹음 시간 조건 관리
- 녹음 완료 후 미리듣기와 저장 흐름 구성
