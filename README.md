...
<p align="center">
  <sub>Android 음성 녹음 상태와 UI 표시 데이터를 분리한 Voice Recorder 구조</sub>
</p>

### Overview

Voice Recorder는 `WBP_VoiceRecorder` Widget과 `VoiceRecorderSubsystem`을 분리해 구성했습니다.  
Subsystem은 녹음 상태, PCM16 데이터, Wave Bar 값, 미리듣기, WAV 저장 흐름을 관리하고, Widget은 Subsystem에서 받은 상태값을 UI에 반영하는 역할을 담당합니다.

AI 보조로 작성한 C++ Subsystem 초안을 Unreal 프로젝트에 맞게 연결하고, Android 모바일 UI 흐름에 맞춰 녹음 시작 / 정지 / 미리듣기 / 저장 단계를 적용했습니다.

### Implementation

| Step | Description |
|------|-------------|
| **1. Recording** | 마이크 입력을 받아 PCM16 데이터로 누적 |
| **2. Wave Bar** | 입력 음량 값을 UI 표시용 Wave Bar 데이터로 변환 |
| **3. Time Control** | 최소 / 최대 녹음 시간 기준으로 정지 가능 여부와 자동 종료 처리 |
| **4. Preview** | 녹음된 데이터를 `SoundWaveProcedural`로 재생해 미리듣기 제공 |
| **5. Save** | Confirm 시 녹음 데이터를 WAV 파일로 저장 |
| **6. UI Update** | `GetUIData`를 통해 Widget에 녹음 상태와 표시 데이터를 전달 |

### Highlights

- `VoiceRecorderSubsystem` 기반으로 녹음 처리와 UI를 분리
- PCM16 녹음 데이터 누적 및 WAV 저장 흐름 구성
- 실시간 Wave Bar와 녹음 완료 후 미리듣기 기능 구현
- 최소 / 최대 녹음 시간 조건 관리
- Widget은 상태 표시와 버튼 입력 처리에 집중하도록 구성
